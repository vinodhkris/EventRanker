...
def parse_nyt():
	url = "http://api.nytimes.com/svc/search/v2/articlesearch.json?q=google&begin_date=20120101&end_date=20141001&page=1&api-key=318a69b2af97848f66071cb4c1fdc831:15:69992102" 
	...
	while i<51 and page<(hits/10):
		...
		try:
			url = "http://api.nytimes.com/svc/search/v2/articlesearch.json?q=google&begin_date=20120101&end_date=20141001&page="+str(page)+"&api-key=318a69b2af97848f66071cb4c1fdc831:15:69992102" 
			response = urlopen(url).read()
			response = json.loads(response)
...
writePickle(articleContent,"googlearticleContent.txt")
